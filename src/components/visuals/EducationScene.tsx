...
export function EducationScene() {
  ...
  return (
    <div ref={root} className="relative flex h-full min-h-64 flex-col">
      <div className="pointer-events-none flex justify-between gap-4 px-0.5 text-[9px] leading-snug tracking-widest text-muted uppercase sm:text-[10px] sm:tracking-[0.14em] md:gap-6 md:tracking-[0.16em]">
        <span className="min-w-0 max-w-[48%]">
          <span className="mb-0.5 block font-display text-xs tracking-normal text-accent md:text-sm">
            {t.highlights.template}
          </span>
          {t.highlights.whitelabel}
        </span>
        <span className="min-w-0 max-w-[48%] text-right">
          <span className="mb-0.5 block font-display text-xs tracking-normal text-secondary md:text-sm">
            {t.highlights.junior}
      ...
      <svg
        viewBox="0 0 400 220"
        className="mt-6 h-full min-h-0 w-full flex-1"
        aria-hidden
      >
