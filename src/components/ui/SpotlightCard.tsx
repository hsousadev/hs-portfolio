...
export function SpotlightCard({
  ...
  return (
    <article ref={root} className="spotlight relative">
      <div className="relative grid items-stretch lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)]">
        ...
        >
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-2 rounded-full border border-border px-2.5 py-1 font-mono text-[10px] tracking-[0.16em] text-muted uppercase">
              <span
                data-dot
                data-gsap
            ...
            <span className="rounded-md border border-accent/50 px-2.5 py-1 font-mono text-[10px] tracking-[0.14em] text-accent uppercase">
              {chip}
            </span>
          </div>

          <div>
            <p className="mb-2 font-mono text-[11px] tracking-[0.16em] text-muted uppercase">
              {role.period[locale]}
            </p>
            <h3 className="font-display text-3xl leading-tight font-semibold tracking-tight text-text md:text-5xl">
              {company}
            </h3>
            <p className="mt-2 font-display text-lg text-text md:text-2xl">
              {title}
            </p>
            <p className="mt-1 text-sm text-muted">{kicker}</p>
          </div>
        ...
        <div
          className={cn(
            "relative min-h-64 overflow-hidden p-4 pb-12 md:min-h-80 md:p-6 md:pb-14",
            reverse && "lg:order-1",
          )}
        >
          <span
            className={cn("spotlight-split", reverse && "is-reverse")}
            aria-hidden
          />
          {visual}
